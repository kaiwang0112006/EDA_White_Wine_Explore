...
```{r Univariate_Plots, warning=FALSE,  message=FALSE, echo=FALSE}
...
wine$num_quality = as.numeric(wine$quality)
ggplot(aes(x = fixed.acidity), data = wine) +
  geom_histogram()+
...
The polygon plot show different distribution of each feature against quality and quality of 5 and 6 have the most variability in all plots. 
...
These plots show how the ratio of fixed acidity and volatile acidity can affect quality. There's not a clear relationship, but the boxplot tells me that the ratio should be above 25(median) to make a high quality wine (quality>5).
...
ggplot(aes(x=residual.sugar, y=alcohol), data = wine) +
  geom_line(aes(color=quality_bucket), stat='summary', fun.y=median)
```

Here is a sugar-alcohol-quality plot, I didn't cut the long tail cause it shows great wine don't have a lot of sugar. They have some sugar and alcohol is high in most cases. But sugar and alcohol are not correlated.
